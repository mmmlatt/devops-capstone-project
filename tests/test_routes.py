...
class TestAccountService(TestCase):
    ...
    def test_read_an_account(self):
        """It should read an account, with good data"""
    ...
    def test_update_an_account(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        new_account = response.get_json()
        new_account["name"] = "This cannot be a name"
    ...
    def test_security_headers(self):
        """It should return security headers when called"""
    ...
    def test_cors(self):
        """It should return Cors policies when called"""
        response = self.client.get("/", environ_overrides=HTTPS_ENVIRON)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        key = "Access-Control-Allow-Origin"
        self.assertEqual(response.headers.get(key), "*")